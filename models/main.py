...
def parse_args():
    ...
    parser.add_argument('--task', type=str, default='reacher', help='which task to train on')
...
def main():
    ...
    if not os.path.isdir('/mnt/data/mamini/models/' + dirname):
        os.makedirs('/mnt/data/mamini/models/' + dirname)
    sess = tf.Session()
    '''
    if args.task == 'pointmass':
      ...
      '''
    train_data_path = "/mnt/data/mamini/models/trajectories/"
    act_scale_coeff = 3.
    


    numberOfTrajectories = len(glob.glob(train_data_path + 'trajectory*'))
    
    all_imgs = []
    all_actions = []    
    for i in range(numberOfTrajectories):
      currentPath  = train_data_path + "trajectory"+str(i+1)+"/"
      numberOfSteps = len(glob.glob(currentPath + '*observation.npy'))
      all_actions.append(np.load(currentPath+'actions.npy').tolist())

      currentStepImages = []
      for i in range(numberOfSteps):
        currentStepImages.append(resize(np.load(currentPath +"step"+str(i)+"observation.npy"),(84,84)).tolist())
    ...
    for batch_idx  in range(args.num_batch_updates):
        ...
        if batch_idx % args.testing_frequency == 0:
            ...
            pickle.dump(object_dump, open('/mnt/data/mamini/models/' + dirname + '/weight_dump_' + str(batch_idx) + '.pkl', 'wb'))
